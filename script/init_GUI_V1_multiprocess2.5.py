...
class RaspiGui(object):
    def __init__(self, init_window_name):
        ...
        self.stop_button = Button(self.runs_button_frame, text='停止采集', font=('行楷', 10), fg="white", bg="#1E90FF",
                                  width=15, command=self.stop_collect)
        self.stop_button.grid(padx=0, pady=5, row=4, column=0)
    ...
    def collect_data(self, duration):
        ...
        self.process1 = process1  # 保存进程对象
        self.process2 = process2
        self.run_log_print(message="脉搏波采集开始:等待" + str(duration) + "秒")
        # 启动两个进程
        process1.start()
        process2.start()
    ...
    def stop_collect(self):
        if self.process1 and self.process1.is_alive():
            ...
            self.process1.join()
            self.run_log_print(message="雷达已停止采集")

        if self.process2 and self.process2.is_alive():
            self.process2.terminate()
            self.process2.join()
            self.run_log_print(message="PPG已停止采集" )
    def get_name(self):
